...
class FitsCatalogTableSink (ISQLSink):
    ...
    def create_table (self, table_name, metadata, file_info):
        if (self._DEBUG):
            print("({}.create_table '{}')".format(self.TOOL_NAME, table_name), file=sys.stderr)
        ...
        # self.store_data(outdata, table_name, db_connection)  # TODO: IMPLEMENT LATER
        db_connection.close()
    ...
    def write_results (self, table_name, outdata, file_info):
    ...
    def write_SQL (self, table_name, outdata, file_info, file_path=None):
        """
