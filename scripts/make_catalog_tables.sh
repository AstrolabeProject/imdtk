...
fits_cat_mktbl_pipe -ff $CONVOS/catalogs/${CAT_NAME} -a /imdtk/config/jwst-cat-aliases.ini -ct ${TBL_NAME} -v
if [ $? -eq 0 ]; then
    fits_cat_table_pipe -ff $CONVOS/catalogs/${CAT_NAME} -ct ${TBL_NAME} -v
fi
